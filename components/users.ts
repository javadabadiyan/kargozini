...
async function handlePut(request: VercelRequest, response: VercelResponse, pool: VercelPool) {
  ...
  try {
    if (password) {
      await pool.sql`
        UPDATE app_users 
        SET username = ${username}, password = ${password}, permissions = ${JSON.stringify(permissions)}
        WHERE id = ${id};
      `;
    } else {
      await pool.sql`
        UPDATE app_users 
        SET username = ${username}, permissions = ${JSON.stringify(permissions)}
        WHERE id = ${id};
      `;
    }
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
    ...
    switch (request.method) {
        case 'GET':
            return await handleGet(response, pool);
        case 'POST':
