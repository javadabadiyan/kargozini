...
interface EditHourlyLogModalProps {
  log: HourlyCommuteReportRow;
  onClose: () => void;
  onSave: (updatedLog: Partial<HourlyCommuteReportRow>) => Promise<void>;
}

const EditHourlyLogModal: React.FC<EditHourlyLogModalProps> = ({ log, onClose, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const baseDate = new Date(log.exit_time || log.entry_time!);
    ...
    let newEntryDate = null;
    if (entryTime.hour && entryTime.minute) {
        newEntryDate = new Date(baseDate.getFullYear(), baseDate.getMonth(), baseDate.getDate(), parseInt(entryTime.hour), parseInt(entryTime.minute));
    }
    
    const updatedLog = {
      log_id: log.log_id,
      exit_time: newExitDate ? newExitDate.toISOString() : null,
      entry_time: newEntryDate ? newEntryDate.toISOString() : null,
      reason: reason,
      ...
      >
        <div className="flex justify-between items-center p-4 border-b dark:border-slate-700">
          <h3 id="modal-title" className="text-xl font-semibold text-gray-800 dark:text-slate-100">
            ویرایش تردد ساعتی برای {log.full_name}
          </h3>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">زمان خروج</label>
            ...
            <div>
              <label htmlFor="reason" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">شرح</label>
...
export default EditHourlyLogModal;