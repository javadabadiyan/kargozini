...
const PersonnelListPage: React.FC = () => {
    ...
    const fetchPersonnel = useCallback(async (page: number, search: string) => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/personnel?type=personnel&page=${page}&pageSize=${pageSize}&searchTerm=${encodeURIComponent(search)}`);
            if (!response.ok) {
    ...
    }, [currentPage, searchTerm, fetchPersonnel]);
    ...
    const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (e) => {
            ...
            // TODO: Add more robust mapping logic if needed
            const mappedData = json.map(row => ({
                personnel_code: row['کد پرسنلی'] || row['کدپرسنلی'],
                first_name: row['نام'],
    ...
    const handleSave = async (personnel: Personnel) => {
        const isNew = !personnel.id;
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('آیا از حذف این پرسنل اطمینان دارید؟')) {
            ...
            try {
                const response = await fetch(`/api/personnel?type=personnel&id=${id}`, { method: 'DELETE' });
                const result = await response.json();
                if (!response.ok) throw new Error(result.details || result.error);
                setStatus({ type: 'success', message: result.message });
                fetchPersonnel(currentPage, searchTerm);
    ...
    const openModalForNew = () => {
        setEditingPersonnel({ id: 0 } as Personnel); // Use a placeholder for new personnel
        setIsModalOpen(true);
    };

    const openModalForEdit = (personnel: Personnel) => {
    ...
    return (
        <div className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-lg p-6 rounded-xl shadow-xl">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-slate-100 mb-6 border-b-2 border-slate-200/50 dark:border-slate-700/50 pb-4">لیست پرسنل</h2>
             ...
             <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700 border dark:border-slate-700">
                    <thead className="bg-gray-100 dark:bg-slate-700/50">
                        <tr>
                            {['کد پرسنلی', 'نام', 'نام خانوادگی', 'کد ملی', 'واحد', 'سمت', 'عملیات'].map(h => <th key={h} className="px-4 py-3 text-right text-xs font-bold text-gray-600 dark:text-slate-300 uppercase">{h}</th>)}
                        </tr>
                    </thead>
                    <tbody className="bg-white dark:bg-slate-800/50 divide-y divide-gray-200 dark:divide-slate-700">
                        {loading ? <tr><td colSpan={7} className="text-center p-4">در حال بارگذاری...</td></tr> :
                         ...
                         personnelList.map(p => (
                            <tr key={p.id}>
                                <td className="px-4 py-3 text-sm">{toPersianDigits(p.personnel_code)}</td>
                                <td className="px-4 py-3 text-sm">{p.first_name}</td>
                                <td className="px-4 py-3 text-sm">{p.last_name}</td>
                                <td className="px-4 py-3 text-sm">{toPersianDigits(p.national_id)}</td>
                                <td className="px-4 py-3 text-sm">{p.department}</td>
                                <td className="px-4 py-3 text-sm">{p.position}</td>
                                <td className="px-4 py-3 text-sm">
                                    <button onClick={() => openModalForEdit(p)} className="p-1 text-blue-600"><PencilIcon className="w-5 h-5"/></button>
                                    <button onClick={() => handleDelete(p.id)} className="p-1 text-red-600 mr-2"><TrashIcon className="w-5 h-5"/></button>
                                </td>
                            </tr>
                         ))}
                    </tbody>
            ...
            <div className="flex justify-between items-center mt-4">
                <span className="text-sm text-gray-700 dark:text-gray-300">نمایش {toPersianDigits(personnelList.length)} از {toPersianDigits(totalCount)} رکورد</span>
            ...
            {isModalOpen && <EditPersonnelModal personnel={editingPersonnel!} onClose={() => setIsModalOpen(false)} onSave={handleSave} />}
        </div>
    );
};
