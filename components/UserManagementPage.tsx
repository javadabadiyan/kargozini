...
export const UserManagementPage: React.FC<UserManagementPageProps> = ({ users, onUsersChange }) => {
    ...
    const handleExcelImport = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                const response = await fetch(`${API_BASE_URL}/api/users?module=admin&action=import`, {
                    method: 'POST',
    ...
    const handleDeleteUser = async (userId: number) => {
        if (window.confirm('آیا از حذف این کاربر اطمینان دارید؟')) {
            try {
                const response = await fetch(`${API_BASE_URL}/api/users?module=admin&id=${userId}`, { method: 'DELETE' });
...
const PasswordModal = ({ user, onClose }: { user: User, onClose: () => void }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/users?module=admin&action=change_password`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
